...
static uint8_t esio_datareg(uint16_t port, int8_t page, uint8_t reg)
{
	OUTB(0xff, port + 4);	/* unlock */
	OUTB(page, port + 4);	/* page */
	OUTB(reg, port + 5);	/* index/register */
	return INB(port + 6);	/* get data */
}

static void dump_esio_data(uint16_t iobase, int page)
{
	uint16_t i;
	...
	for (i = 0; i < 256; i++) {
		if (i % 16 == 0)
			printf("\n%02x: ", i);
		printf("%02x ", esio_datareg(iobase, page, i));
	}
	printf("\n");
}
...
extra:
	if (extra_dump && iobase) {
		...
		case 0xc910:	/* NCT6792D */
		case 0xd120:	/* NCT6793D */
		case 0xd350:	/* NCT6795D, NCT6795D-M */
			maxpage = 0x0a;
			break;
		case 0xd420:	/* NCT6796D */
			maxpage = 0x0c;
			break;
		case 0xc730:	/* NCT6683D */
			for (i = 0; i < 16; i++)
				dump_esio_data(iobase, i);
			break;
		}
		for (i = 0; i <= maxpage; i++)
